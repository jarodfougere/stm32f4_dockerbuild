/* --------------------------------------------------------------------------
 * Portions Copyright � 2019 STMicroelectronics International N.V. All rights
 *reserved. Copyright (c) 2013-2019 Arm Limited. All rights reserved.
 *
...
#define IS_IRQ()                                                               \
    (IS_IRQ_MODE() || (IS_IRQ_MASKED() && (KernelState == osKernelRunning)))
...
__STATIC_INLINE void SVC_Setup(void)
{
     ...
     * under:https://github.com/ARM-software/CMSIS-FreeRTOS/issues/35 until it
     * is correctly fixed, the code below is commented
     */
    ...
    if (!IS_IRQ() && (func != NULL))
    {
        ...
        if (mem == 1)
        {
            hTask = xTaskCreateStatic(
                (TaskFunction_t)func, name, stack, argument, prio,
                       ...
                       const osTimerAttr_t *attr)
{
    ...
    if (!IS_IRQ() && (func != NULL))
    {
        ...
        if (callb != NULL)
        {
            ...
            {
                hTimer =
                    xTimerCreateStatic(name, 1, reload, callb, TimerCallback,
                             ...
                             uint8_t msg_prio, uint32_t timeout)
{
    ...
    else
    {
        ...
        {
            if (xQueueSendToBack(hQueue, msg_ptr, (TickType_t)timeout) !=
                pdPASS)
            {
                if (timeout != 0U)
                {
